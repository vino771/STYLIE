...
import { ArrowLeft, Plus, X, ImagePlus, Check } from 'lucide-react';
import { ALL_COLLECTIONS, useStylie } from '@/context/StylieContext';
...
function InputField({ label, ...props }: { label: string } & React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <input
        {...props}
        className="bg-white/5 border border-white/10 text-white placeholder:text-white/20 rounded-xl px-4 py-3 text-xs font-medium focus:outline-none focus:border-[#FF5A1F]/60 focus:ring-1 focus:ring-[#FF5A1F]/30 transition-all"
      />
    </div>
  );
}
...
export default function AdminNewProduct() {
  const { adminAddProduct, enabledCollections } = useStylie();
  const router = useRouter();
  const collectionOptions = enabledCollections.length > 0 ? enabledCollections : ALL_COLLECTIONS;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!title || !brand || !collectionCategory || !price || !stock || selectedSizes.length === 0 || selectedColors.length === 0) return;
    ...
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <h1 className="font-poppins font-extrabold text-2xl text-white tracking-tight">Add New Product</h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-8">
        ...
        <div className="bg-white/5 border border-white/10 rounded-[24px] p-6 flex flex-col gap-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <InputField label="Product Title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Classic Tailored Blazer" required />
            <InputField label="Brand Name" value={brand} onChange={(e) => setBrand(e.target.value)} placeholder="e.g. STYLIE ATELIER" required />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            ...
            <InputField label="Price (₹)" type="number" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="e.g. 12999" required min="0" />
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Collection Category</label>
              <select
                ...
                className="bg-white/5 border border-white/10 text-white rounded-xl px-4 py-3 text-xs font-medium focus:outline-none focus:border-[#FF5A1F]/60 appearance-none"
              >
                <option value="" className="bg-[#0E1117]">Select Collection</option>
                {collectionOptions.map((c) => (
                  <option key={c} value={c} className="bg-[#0E1117]">{c}</option>
                ))}
              </select>
            </div>
            <InputField label="Discount (%)" type="number" value={discount} onChange={(e) => setDiscount(e.target.value)} placeholder="0" min="0" max="90" />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <InputField label="Stock Quantity" type="number" value={stock} onChange={(e) => setStock(e.target.value)} placeholder="e.g. 50" required min="0" />
          </div>
          <div className="flex flex-col gap-1.5">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Materials & Care Details (one per line)</label>
            <textarea
              value={details}
        ...
        <div className="bg-white/5 border border-white/10 rounded-[24px] p-6 flex flex-col gap-4">
          ...
          {images.length === 0 && (
            <div className="flex items-center gap-3 text-white/20 text-xs py-4">
              <ImagePlus className="w-5 h-5" />
              Add at least one image URL above. A default placeholder will be used if none are provided.
            </div>
          )}
        </div>
        ...
        <div className="flex gap-4">
          ...
          >
            {success ? (
              <><Check className="w-4 h-4" /> Product Added! Redirecting...</>
            ) : (
